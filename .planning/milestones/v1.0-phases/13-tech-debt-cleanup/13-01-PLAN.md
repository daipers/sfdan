...
must_haves:
  truths:
    - "Deployment Platform plugin removed from dependencies"
...
<objective>
Clean up tech debt from the GitHub Pages migration: remove unused Deployment Platform plugin, verify Supabase security, and resolve code placeholder.
</objective>
...
<task type="auto">
  <name>Task 1: Remove Deployment Platform plugin from dependencies</name>
  <files>package.json</files>
  <action>
    Remove "@deployment/plugin-nextjs" from devDependencies:
    - Run: npm uninstall @deployment/plugin-nextjs
    - Or manually remove from package.json and run npm install
  </action>
  <verify>npm run build succeeds</verify>
  <done>package.json no longer contains @deployment/plugin-nextjs</done>
</task>
...
<success_criteria>
- Task 1: Deployment Platform plugin removed
