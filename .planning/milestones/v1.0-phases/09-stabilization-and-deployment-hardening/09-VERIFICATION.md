...
human_verification:
  ...
  - test: "Blue/green promotion with health-gated rollback in Deployment Platform"
    expected: "Staging deploys require manual approval to promote; failed /api/health triggers rollback via Deployment Platform"
    why_human: "Depends on Deployment Platform dashboard settings, deploy hooks, and rollback behavior"
...
| 7 | Deploys fail when tests fail prior to build | ✓ VERIFIED | `deployment.toml` build command runs `npm run test:run && npm run build` |
...
| `deployment.toml` | Build command with test gate | ✓ VERIFIED | `npm run test:run && npm run build` |
...
| `docs/operations/release-workflow.md` | Deployment Platform rollback | auto-rollback step | WIRED | Rollback guidance documented |
| `deployment.toml` | `npm run test:run` | build command | WIRED | Build command includes `test:run` |

### Requirements Coverage

| Requirement | Status | Blocking Issue |
| --- | --- | --- |
| STAB-01 | ✓ VERIFIED | Human verification approved for Deployment Platform manual promotion + rollback |
...
2. **Blue/green promotion with health-gated rollback in Deployment Platform**

**Test:** Promote a staging deploy to production and simulate `/api/health` failure.
**Expected:** Manual approval gate exists; failed health checks trigger rollback via Deployment Platform.
**Why human:** Depends on Deployment Platform UI settings and deploy hook behavior.
