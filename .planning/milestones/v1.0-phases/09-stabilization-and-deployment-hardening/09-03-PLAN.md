  ...
  - service: deployment
    ...
    dashboard_config:
      - task: "Create a staging environment with its own env vars"
        location: "Deployment Platform -> Site settings -> Build & deploy -> Environment"
      - task: "Require manual promotion for production deployments"
        location: "Deployment Platform -> Deploys -> Production deploys"
...
must_haves:
  ...
  key_links:
    - from: "docs/operations/release-workflow.md"
      ...
      pattern: "api/health"
    - from: "docs/operations/release-workflow.md"
      to: "Deployment Platform rollback"
      via: "auto-rollback step"
      pattern: "rollback"
    - from: "deployment.toml"
      to: "npm run test:run"
      via: "build command"
      pattern: "test:run"
---
...
Output: Release workflow docs, incident templates, and Deployment Platform build protection updates.
</objective>
...
<task type="auto">
  <name>Enforce build protections on Deployment Platform</name>
  <files>deployment.toml</files>
  <action>
Update the Deployment Platform build command to run unit tests before build (use `npm run test:run && npm run build`). Keep the Node version setting intact and avoid adding E2E tests to the build step.
  </action>
  <verify>rg "test:run" deployment.toml</verify>
  <done>Deployment Platform build now fails if unit tests fail.</done>
</task>

</tasks>

<verification>
- `deployment.toml` includes a test gate in the build command
