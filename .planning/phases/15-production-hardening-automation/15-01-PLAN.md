...
<task type="auto">
  ...
  <action>
    ...
    - **EmailGateForm**: Update `handleSubmit` to call `supabase.auth.signInWithOtp({ email, options: { emailRedirectTo: window.location.origin + '/auth/callback?redirectTo=' + encodeURIComponent(window.location.pathname) } })` followed by `supabase.from('leads').insert({ email })`. This ensures the session is established by the callback handler (15-04) before returning the user to the gated content.
