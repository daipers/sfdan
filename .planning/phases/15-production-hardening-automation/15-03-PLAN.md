...
must_haves:
  truths:
    ...
    - "Content 'unlocks' dynamically on the client after authentication"
  ...
  artifacts:
    ...
    - path: "src/app/content/[slug]/page.tsx"
      provides: "Client-side gated content re-hydration"
...
<task type="auto">
  ...
  <action>
    ...
    - Test the full journey: 
      ...
      - Authenticated user visiting the same page -> accesses content successfully (verifying client-side re-hydration).
  ...
  <done>
    Critical lead generation and gated content journeys are verified for production, including client-side unlocking.
  </done>
</task>

<task type="auto">
  <name>Task 4: Implement Client-side Gating Re-hydration</name>
  <files>
    - src/app/content/[slug]/page.tsx
  </files>
  <action>
    - Update `src/app/content/[slug]/page.tsx` to handle authentication states on the client.
    - Because the site is statically exported, gating logic evaluated at build time will always default to the anonymous (gated) state.
    - Add a `useEffect` hook to check the current session using `supabase.auth.getSession()` on mount.
    - Listen for auth changes using `supabase.auth.onAuthStateChange()`.
    - If a valid session is found, dynamically "unlock" the content by updating local component state.
    - Ensure a smooth transition from gated placeholder to full content without layout shifts if possible.
  </action>
  <verify>
    Run `npm run build && npx serve out` (or similar static server). Visit a gated report anonymously, log in, and verify the content unlocks without a full page reload or server-side re-render.
  </verify>
  <done>
    Gated content correctly unlocks on the static host based on the client-side auth session.
  </done>
</task>
